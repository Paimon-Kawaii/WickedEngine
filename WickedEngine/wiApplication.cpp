...
namespace wi
{
	...
	void Application::Run()
	{
		...
		if (!startup_script)
		{
			...
			std::string startup_luac_filename = wi::helper::GetCurrentPath() + "/startup.luac";
			if (wi::helper::FileExists(startup_luac_filename))
			{
				if (wi::lua::RunBinaryFile(startup_luac_filename))
				{
					wi::backlog::post("Executed startup file: " + startup_luac_filename);
				}
			}
