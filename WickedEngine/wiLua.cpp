...
namespace wi::lua
{
	...
	int Internal_DoFile(lua_State* L)
	{
		...
		else wi::lua::SError(L, "execfile(string filename) not enough arguments!");
	...
	int Internal_DoBinaryFile(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			wi::lua::SError(L, "execbinaryfile(string filename): File could not be read!");
		}
		else wi::lua::SError(L, "execbinaryfile(string filename): Not enough arguments!");
		return 0;
	}
	int Internal_CompileBinaryFile(lua_State* L)
	{
		...
		else wi::lua::SError(L, "compilebinaryfile(string filename_src, filename_dst): Not enough arguments!");
		return 0;
	}
	...
	void Initialize()
	{
		...
		RegisterFunc("execfile", Internal_DoFile);
		RegisterFunc("execbinaryfile", Internal_DoBinaryFile);
	...
	bool CompileText(const char* script, wi::vector<uint8_t>& dst)
	{
		...
		dst.clear();
		if(lua_dump(lua_internal().m_luaState, writer, &dst, 1) != LUA_OK)
		{
			PostErrorMsg();
