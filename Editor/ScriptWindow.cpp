		...
		else
		{
			...
			wi::helper::FileDialog(params, [=](std::string filename) {
				wi::eventhandler::Subscribe_Once(wi::eventhandler::EVENT_THREAD_SAFE_POINT, [=](uint64_t userdata) {
					...
					std::string temp = filename.substr(filename.find_last_of('.'));
					if (temp == ".lua")
						temp = filename + "c";
					else temp = filename;
					fileButton.SetText(wi::helper::GetFileNameFromPath(temp));
					});
				});
		}
	});
...
void ScriptWindow::SetEntity(wi::ecs::Entity entity)
{
	...
	if (script != nullptr)
	{
		if (!script->filename.empty())
			script->CreateFromFile(script->filename);
		if (script->resource.IsValid())
			fileButton.SetText(wi::helper::GetFileNameFromPath(script->filename));
		else fileButton.SetText("Open File...");
