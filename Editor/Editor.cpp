...
void EditorComponent::Load()
{
			...
			wi::helper::FileDialog(params, [&](std::string filename) {
				wi::eventhandler::Subscribe_Once(wi::eventhandler::EVENT_THREAD_SAFE_POINT, [=](uint64_t userdata) {
					...
					if (!extension.compare("LUA"))
					{
						...
						last_script_path = file_name + "c";
						main->config.Set("last_script_path", last_script_path);
						main->config.Commit();
						playButton.SetScriptTip("execbinaryfile(\"" + last_script_path + "\")");
						wi::lua::RunText("compilebinaryfile('" + file_name + "', '" + last_script_path + "')");
						wi::lua::RunBinaryFile(last_script_path);
					}
				});
			});
		}
	...
	playButton.SetScriptTip("dobinaryfile(\"" + last_script_path + "\")");
...
void EditorComponent::Open(std::string filename)
{
	...
	if (type == FileType::LUA)
	{
		...
		wi::lua::RunBinaryFile(last_script_path);
		componentsWnd.RefreshEntityTree();
		RegisterRecentlyUsed(last_script_path);
		return;
	}
